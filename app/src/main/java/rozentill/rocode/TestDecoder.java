...
public class TestDecoder extends Activity implements SurfaceHolder.Callback { // 定义对象
    ...
    private SurfaceHolder mSurfaceHolder = null; // SurfaceHolder 对象:(抽象接 口)SurfaceView 支持类
...
//// this.setResult(RESULT_OK, resultIntent); // } catch (Exception e) {
//
// }
//            e.printStackTrace();
                int previewWidth =
...
// 当 SurfaceView/预览界面的格式和大小发生改变时,该方法被调用
    {
        Log.i("TAG", "SurfaceHolder.Callback:Surface Changed"); //mPreviewHeight = height;
//mPreviewWidth = width;
        initCamera();
    ...
    /*【2】【相机预览】*/
    private void initCamera()//surfaceChanged 中调用
    {
...
//        format for picture 设定相片格式为 JPEG,默认为 NV21
                parameters.setPreviewFormat(PixelFormat.YCbCr_420_SP); //Sets
//        the image format for preview picture,默认为 NV21 /*【ImageFormat】JPEG/NV16(YCrCb format,used for Video)/NV21(YCrCb
//format,used for Image)/RGB_565/YUY2/YU12*/
// 【调试】获取 camera 支持的 PictrueSize,看看能否设置??
                List<Camera.Size> pictureSizes = mCamera.getParameters().getSupportedPictureSizes();
                List<Camera.Size> previewSizes = mCamera.getParameters().getSupportedPreviewSizes();
                List<Integer> previewFormats = mCamera.getParameters().getSupportedPreviewFormats();
                List<String> focusModes = parameters.getSupportedFocusModes(); // List<Integer> previewFrameRates =
...
// parameters.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE);//1 连 续对焦
                parameters.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO);//自动对 焦
// parameters.setPreviewFpsRange(29950, 30000);
// 设置拍照和预览图片大小
// parameters.setPictureSize(4160, 2336); //honor 7 指定拍照图片的大小
// parameters.setPictureSize(2048, 1536); //nubia Z7 MAX 指定拍照图 片的大小
// parameters.setPictureSize(3264, 1836); //nexus4 指定拍照图片的大小
                parameters.setPictureSize(1280, 720); //最优拍照图片的大小
                parameters.setPreviewSize(mPreviewHeight, mPreviewWidth); // 指定
//        preview 的大小
// parameters.setPreviewFrameRate(20);
//这两个属性 如果这两个属性设置的和真实手机的不一样时,就会报错 // 横竖屏镜头自动调整
                if (this.getResources().getConfiguration().orientation != Configuration.ORIENTATION_LANDSCAPE)
                {
                    parameters.set("orientation", "portrait"); //
                    parameters.set("rotation", 90); // 镜头角度转 90 度(默认摄像头是横拍)
                    mCamera.setDisplayOrientation(90); // 在 2.2 以上可以使用
                } else// 如果是横屏
                {
